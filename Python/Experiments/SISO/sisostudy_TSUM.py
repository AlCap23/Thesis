...
def experimental_setup():
	...
	filename = 'SISO_06102017_TSum.csv'
	...
	T = np.random.uniform(lag_limits[0],lag_limits[1],(sample_size,max_order))
	...
	for samples in range(0,sample_size):
		...
		dist = np.sort(T[samples,:degree])
		dist = TSum*dist/np.sum(dist)
	...
	columns = ['Sample No.', 'Order', 'K', 'TDom', 'L', 'KM', 'TM', 'LM', 'MS Real', 'MS Ideal', 'MT Real', 'MT Ideal']
...
def experiment(Num,Den,L,R,noise_limit,sample_size,max_order,filename, columns):
	...
	for sample in range(0,sample_size):
		...
		# Append Data
		#if order == 1:
		R.loc[sys_no-1] = [sys_no, current_order, Num[sample]/Den[sample,current_order], np.max(np.abs(np.real(np.polynomial.polynomial.polyroots(Den[sample,:current_order+1])))),L[sample], km, tm, lm, MS_Real, MS_Iden, MT_Real, MT_Iden]
		#else:
		...
		per = float(sample/sample_size)*100
