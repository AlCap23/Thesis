...
def experiment(num, den, l, R, filename, sample_size, max_deg, dt, t_sim, H, wmin, wmax, dw, w_special):
	...
	for samples in range(0, sample_size):
		print("Percentage: "+str(float(samples/sample_size)*100))
		...
		# sim.clear()
		# Load new model
		sim = fmi.load_fmu("./2_2_n"+str(degree)+"/Masterthesis_Models_mimo_0processmodel.fmu")
	...
	###########################################################
		...
		for outputs in range(0,2):
			...
			for inputs in range(0,2):
				# Set system gain
				params_name.append("num["+str(outputs+1)+","+str(inputs+1)+",1]")
				params.append(num[samples][outputs][inputs])
				#params.update({"fmu.num["+str(outputs+1)+","+str(inputs+1)+",1]": num[samples][outputs][inputs]})
				# Set system delay
				params_name.append("delay["+str(outputs+1)+","+str(inputs+1)+"]")
				params.append(l.item(samples,outputs,inputs))
				
				#params.update({"fmu.delay["+str(outputs+1)+","+str(inputs+1)+"]": l.item(samples,outputs,inputs)})
				# Loop over denominator coefficients
				for order in range(0, degree+1):
					params_name.append("den["+str(outputs+1)+","+str(inputs+1)+","+str(degree-order+1)+"]")
					params.append(den[samples][outputs][inputs][(order)])
					#params.update({"fmu.den["+str(outputs+1)+","+str(inputs+1)+","+str(degree-order+1)+"]": den[samples][outputs][inputs][(order)]})

		# Set the parameter
		#print(sim.get)
		sim.set(params_name, params)
		# Show the Parameter
		#sim.showParameterDialog()
		# Store the state space rep for later use
		sim.setup_experiment(start_time = 0)
		sim.initialize()
		ss = sim.get_state_space_representation()
		print(ss[0])
	...
	###########################################################
		...
		res = sim.simulate(final_time = t_sim)
