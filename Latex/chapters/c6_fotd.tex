\chapter{Control of TITO FOTD Processes}
\label{c:fotd}
...
Eq. \ref{c:fotd:e:Combination} reuses the experimental data to approximate the systems output. $y_{ii}$ is the i-th output of the system reacting to excitation via the i-th input. Hence the data can be used for FOTD identification as presented earlier.\\
...
With parameters $K_{P,i},K_{I,i} \in \mathbb{R}$ are calculated via the AMIGO Tuning Rules as given earlier. Since the approximation given in Eq.\ref{c:fotd:e:exampleFOTDapprox} holds an inevitable error so do the parameter. \\
...
From Eq. \ref{c:fotd:e:exampleTransformedCoupling} the dependency of the coupling on the both the static gain of the system and the dynamical behaviour can be observed. This coincides with the statements of LUNZE ZITIEREN.\\
...
Solving Eq. \ref{c:fotd:e:Hmax*} for the set point weighting controller holds:
...
To detune the controller solving Eq. \ref{c:fotd:e:Hmax*} for the integral controller as before holds:
...
Roosenbrooks function as an Example!
