...
Info(steps,samples) = struct('Kappa',1,'RGA',1,'Q',1,'G',1);
...
for loop = 1:1:steps+1
    for sample = 1:1:samples+1
        ...
        Info(loop,sample).Kappa = gainfactor/k(2);
        ...
        %% Get the information
        Info(loop,sample).RGA = stepinfo(CL1);
        Info(loop,sample).Q = stepinfo(CL2);
        Info(loop,sample).G = stepinfo(CL3);
    end
end