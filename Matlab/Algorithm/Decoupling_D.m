function C = Decoupling_D(G,Constrains,Method,SetPointWeight)
...
% Redefine the Splitter
S = tf();
for inputs= 1:sys_size(1)
    for outputs = 1:sys_size(2)
        if inputs ~= outputs
            S(inputs,outputs) = -K(inputs,outputs)/K(inputs,inputs)*tf([1],[T(inputs,outputs)+L(inputs,outputs),1]);
        end
    end
end
