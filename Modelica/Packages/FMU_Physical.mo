...
package FMU_Physical "Package for Physical Systems"
  package HP_HT_Control
    ...
    equation
      ...
      connect(multivariable_Controller.u_s[2], add_PR.y) annotation (Line(
            points={{8.88178e-016,77},{22,77},{22,46},{53,46}}, color={0,0,127}));
      connect(T_R.y, add_TR.u2)
        annotation (Line(points={{105,94},{88,94},{74,94}}, color={0,0,127}));
      connect(multivariable_Controller.u_s[1], add_TR.y) annotation (Line(
            points={{8.88178e-016,79},{26,79},{26,88},{51,88},{51,88}}, color={
              0,0,127}));
      connect(add_TR.u1, step_TR.y) annotation (Line(points={{74,82},{90,82},{
              90,72},{105,72}}, color={0,0,127}));
      connect(add_PR.u1, step_PR.y) annotation (Line(points={{76,40},{92,40},{
              92,30},{109,30}}, color={0,0,127}));
      annotation (Icon(coordinateSystem(preserveAspectRatio=false)), Diagram(
            coordinateSystem(preserveAspectRatio=false)));
